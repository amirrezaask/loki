...
impl Parser {
    ...
    fn new_entity_idx(&self) -> Index {
        let mut rng = rand::thread_rng();
        let id: u64 = rng.gen();;
        return id;
    }
    fn new_node(&mut self, id: Index, data: NodeData, start_line: usize, start_col: usize) -> Node {
        self.node_counter += 1;
        let node = Node {
            id,
            data,
    ...
    fn expect_definition_declaration_assignment(&mut self) -> Result<Node> {
        ...
        match self.current_token().ty {
            ...
            TokenType::DoubleColon => {
                self.forward_token();
                let rhs = self.expect_expr()?;
                let dest_clone = self.hir.get_node(dest.id.clone())?;
    ...
    */
        ...
        match self.current_token().ty {
            TokenType::OpenBrace => match ty.data {
                ...
                _ => {
                    ...
                    if is_struct_init {
                        self.forward_token();
                        let mut fields = Vec::<(Index, Index)>::new();
                        loop {
    ...
    fn expect_type_expression(&mut self) -> Result<Node> {
        match self.current_token().ty {
            ...
            TokenType::KeywordEnum => {
                ...
                self.expect_token(TokenType::OpenBrace)?;
                self.forward_token();
                let mut variants = Vec::<Index>::new();
    ...
    fn expect_block(&mut self) -> Result<Index> {
        if self.current_token().ty == TokenType::OpenBrace {
            self.forward_token()
        }
        let mut stmts = Vec::<Index>::new();
