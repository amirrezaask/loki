...
impl IR {
    ...
    fn find_identifier_type(&self, dependent_node: NodeIndex, mut scope: NodeIndex, identifier: String) -> Option<Type> {
        let mut scope_node = self.nodes.get(&scope).unwrap().clone();
        loop {
        ...
        unreachable!();
        // return Err(Error::TypeCheckError(TypeCheckError::DependencyToIdentifier{
        //     identifier,
    ...
    fn type_expression(&mut self, expression_index: NodeIndex) -> Result<Option<Type>> {
            ...
            NodeData::Expression(ref expr) => {
                match expr {
                    ...
                    Expression::Identifier(ref identifier) => {
                        let identifier_type = self.find_identifier_type(expression_index, node.parent_block.unwrap(), identifier.clone());
                    ...
                    Expression::Function { ref args, ret_ty: ref ret_ty_node, ref body } => {
                        for arg in args {
                            let arg_type = self.type_statement(*arg)?;
                            if arg_type.is_none() {
                        ...
                        let body_type = self.type_scope(*body)?;
    ...
    fn type_statement(&mut self, stmt_index: NodeIndex) -> Result<Option<Type>> {
            ...
            NodeData::Statement(ref stmt) => {
                match stmt {
                    ...
                    Statement::Assign { ref lhs, ref rhs } => {
                        let lhs_type = self.type_expression(*lhs)?;
                        let rhs_type = self.type_expression(*rhs)?;
                        if lhs_type.is_none() {
                    ...
                    Statement::If { ref cases } => {
                        for case in cases {
                            ...
                            let scope_type = self.type_scope(case.1)?;
                    ...
                    Statement::For { ref start, ref cond, ref cont, ref body } => {
                        let start_type = self.type_statement(*start)?;
                        ...
                        let body_type = self.type_statement(*body)?;
                        if body_type.is_none() {
                    ...
                    Statement::ForIn { ref iterator, ref iterable, ref body } => {
                        let iterator_type = self.type_expression(*iterator)?;
    ...
    pub fn type_root(&mut self) -> Result<()> {
        self.type_scope(self.root)?;
        Ok(())
    }
    fn type_scope(&mut self, scope_index: NodeIndex) -> Result<Option<Type>> {
        ...
        if let NodeData::Statement(Statement::Scope { owner, is_file_root, ref stmts }) = scope.data {
            for stmt in stmts {
                let ty = self.type_statement(*stmt)?;
                if ty.is_none() {
